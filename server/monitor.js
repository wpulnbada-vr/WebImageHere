...
function getCpuPercent() {
  ...
  const percent = ((usage.user + usage.system) / elapsed) * 100;
...
function getDiskUsage() {
  ...
  try {
    if (process.platform !== 'win32') {
      ...
      if (parts.length >= 2) {
        diskFreeMB = parseInt(parts[0]) || 0;
        diskTotalMB = parseInt(parts[1]) || 0;
      }
    }
  } catch {}

  const data = {
    downloadsSizeMB: Math.round(totalSize / (1024 * 1024)),
    downloadsFiles: totalFiles,
    downloadsFolders: totalFolders,
    diskFreeMB,
    diskTotalMB,
  };
...
function collectSystemMetrics(jobs, queue, serverStartTime) {
  const mem = process.memoryUsage();
  const disk = getDiskUsage();

  let running = 0, queued = 0, activeBrowsers = 0;
  for (const job of jobs.values()) {
    if (job.status === 'running') {
      running++;
      if (job.scraper) activeBrowsers++;
    }
  ...
  return {
    ...
    disk,
    puppeteer: { activeBrowsers, maxConcurrent: 2 },
    queue: { running, queued: queue.length },
  };
}
...
function aggregateStats(history) {
  ...
  return {
    overview: {
      totalJobs: total,
      successRate: total > 0 ? Math.round((completed.length / total) * 1000) / 10 : 0,
      totalImages,
...
function getRealtimeStatus(jobs, queue, history) {
  const running = [];
  const queued = [];
  for (const job of jobs.values()) {
    if (job.status === 'running') {
      running.push({
        ...
        lastEvent: job.lastEvent, eventsCount: job.events.length, startedAt: job.startedAt,
      });
    } else if (job.status === 'queued') {
      queued.push({ id: job.id, url: job.url, keyword: job.keyword, createdAt: job.createdAt });
    }
  }
